...
use linux_loader::{
    ...
    loader::{elf::Elf, bzimage::BzImage, KernelLoader},
};
use log::{debug, info};
use vm_memory::{Address, Bytes, GuestAddress, GuestMemoryMmap};

use crate::virtio::{MmioTransport, ConsoleDevice};
use std::io::{Write, Read};
use std::sync::{Arc, Mutex};
use std::thread;
use std::time::Duration;
use crate::{lapic, regs, sys, Machine, Vcpu, VmExit};
...
impl Linux64Guest {
    ...
    pub fn load<'a>(&self, machine: &'a mut Machine) -> anyhow::Result<(GuestMemoryMmap, Vcpu<'a>)> {
        // 1. Setup Memory
        ...
        // Try ELF
        let (entry_point, _is_long_mode) = if let Ok(res) = Elf::load(&guest_mem, None, &mut kernel_file, Some(GuestAddress(HIMEM_START))) {
             info!("Detected ELF Kernel. Entry: 0x{:x}", res.kernel_load.raw_value());
             (res.kernel_load.raw_value(), true)
        } else {
            kernel_file.seek(SeekFrom::Start(0))?;
            let res = BzImage::load(&guest_mem, None, &mut kernel_file, Some(GuestAddress(HIMEM_START)))?;
            info!("Detected BzImage Kernel. Entry: 0x{:x}", res.kernel_load.raw_value());
        ...
        guest_mem.write_slice(unsafe { 
            std::slice::from_raw_parts(gdt_table.as_ptr() as *const u8, 40) 
        ...
        // Flags for set_state
        let flags = regs::STATE_GPRS | regs::STATE_SEGS | regs::STATE_CRS | 
                    regs::STATE_MSRS | regs::STATE_FPU | regs::STATE_INTR; 
    ...
    pub fn run<'a>(&self, vcpu: &mut Vcpu<'a>, guest_mem: &GuestMemoryMmap) -> anyhow::Result<()> {
        info!("Starting VCPU...");
        ...
        // Initialize VirtIO
        let virtio_console = Arc::new(Mutex::new(MmioTransport::new(Box::new(ConsoleDevice::new()))));
        ...
        let mut apic_base: u64 = lapic::APIC_BASE | 0x800; // Base + Enable bit

        loop {
            // Main VCPU Loop
            // println!("Entering VCPU Run");
            let exit = vcpu.run()?;
            // let rip = vcpu.get_rip().unwrap_or(0);
            // println!("Exited VCPU Run: {:?} at RIP={:#x}", exit, rip);
            match exit {
                VmExit::Io { port, is_in, ref data, npc } => {
                    if is_in {
                        let val: u8;
                        if (0x40..=0x43).contains(&port) || port == 0x61 {
                            val = pit.lock().unwrap().read(port);
                            // println!("PIT Read Port {:x} -> {:x}", port, val);
                             ...
                             // println!("PIC Master Read {:x} -> {:x}", port, val);
                        ...
                        let mut state = vcpu.get_state(regs::STATE_GPRS)?;
                        state.gprs[regs::GPR_RAX] = (state.gprs[regs::GPR_RAX] & !0xFF) | (val as u64);
                        state.gprs[regs::GPR_RIP] = npc;
                        vcpu.set_state(&state, regs::STATE_GPRS)?;
                    } else {
                        if (0x3F8..=0x3FF).contains(&port) {
                           serial.write(port - 0x3F8, data[0]);
                            ...
                            if !data.is_empty() {
                                ...
                                // println!("PIT Write Port {:x} Val {:x}", port, data[0]);
                            }
                        } else if (0x20..=0x21).contains(&port) {
                            if !data.is_empty() {
                                let val = data[0];
                                master_pic.lock().unwrap().io_write((port - 0x20) as u8, val);
                                if port == 0x21 {
                                    // println!("PIC Master Mask Update / Data Write: val={:x}", val);
                                } else {
                                     // println!("PIC Master Cmd: {:x}", val);
                                }
                            }
                        } else if (0xA0..=0xA1).contains(&port) {
                            if !data.is_empty() {
                                slave_pic.lock().unwrap().io_write((port - 0xA0) as u8, data[0]);
                                if port == 0xA1 {
                                     // println!("PIC Slave Mask Update / Data Write: val={:x}", data[0]);
                                }
                            }
                        } else {
                            match port {
                        ...
                        let current_rip = vcpu.get_rip().unwrap_or(0);
                        vcpu.advance_rip(npc.wrapping_sub(current_rip))?;
                    }
                },
                VmExit::Memory { gpa, is_write, inst_len } => {
                    ...
                    if gpa >= base && gpa < base + 0x1000 {
                        ...
                        if is_write {
                            // TODO: Proper instruction decoding to find source register.
                            // For now, assuming standard compiler output using EAX/RAX.
                            let val = state.gprs[regs::GPR_RAX] as u32;
                            debug!("LAPIC Write: offset={:#x} val={:#x} at RIP={:#x}", offset, val, rip);
                            lapic.lock().unwrap().write(offset, val);
                        } else {
                            let val = lapic.lock().unwrap().read(offset);
                            debug!("LAPIC Read: offset={:#x} val={:#x} at RIP={:#x}", offset, val, rip);
                            

                            // TODO: Proper decoding to find destination. Assuming EAX.
                            // Masking high bits of RAX? Usually 32-bit access.
                            state.gprs[regs::GPR_RAX] = (state.gprs[regs::GPR_RAX] & !0xFFFFFFFF) | (val as u64);
                            vcpu.set_state(&state, regs::STATE_GPRS)?;
                        }
                        
                        vcpu.advance_rip(inst_len as u64)?;
                    } else if (0xd0000000..0xd0000200).contains(&gpa) {
                        let offset = gpa - 0xd0000000;
                        debug!("VirtIO MMIO Access: offset={:#x}, write={}", offset, is_write);
                        let mut state = vcpu.get_state(regs::STATE_GPRS)?;
                        ...
                        let rip = vcpu.get_rip().unwrap_or(0);
                        debug!("Unhandled Memory Exit: gpa={:#x}, is_write={}, len={} at RIP={:#x}", gpa, is_write, inst_len, rip);
                        break;
                    }
                },
                VmExit::RdMsr { msr, npc } => {
                    let val = if msr == 0x1B {
                        ...
                        debug!("Unhandled RDMSR: {:#x}", msr);
                        0
                    };
                    let mut state = vcpu.get_state(regs::STATE_GPRS)?;
                    state.gprs[regs::GPR_RAX] = val & 0xFFFFFFFF;
                    state.gprs[regs::GPR_RDX] = val >> 32;
                    state.gprs[regs::GPR_RIP] = npc;
                    vcpu.set_state(&state, regs::STATE_GPRS)?;
                },
                VmExit::WrMsr { msr, val, npc } => {
                    if msr == 0x1B {
                        ...
                        debug!("Unhandled WRMSR: {:#x} Val={:#x}", msr, val);
                    }
                    let current_rip = vcpu.get_rip().unwrap_or(0);
                    vcpu.advance_rip(npc.wrapping_sub(current_rip))?;
                },
                VmExit::Shutdown => {
                    let rip = vcpu.get_rip().unwrap_or(0);
                    log::error!("VM Shutdown at RIP={:#x}", rip);
                    break;
                },
                VmExit::Unknown(0) => {
                    // NVMM_EXIT_NONE: Triggered by os_return_needed() for signal/debug checks.
                    // Just continue to re-enter Guest.
                }
                VmExit::Unknown(reason) => {
                    let rip = vcpu.get_rip().unwrap_or(0);
                    debug!("Unknown Exit {:#x} at RIP={:#x}", reason, rip);
                    break;
                }
            }
        }
        ...
        // Dump State on Exit
        let state = vcpu.get_state(regs::STATE_GPRS | regs::STATE_CRS | regs::STATE_SEGS)?;
        log::error!("--- VCPU STATE AT EXIT ---");
        log::error!("RIP: {:#x}", state.gprs[regs::GPR_RIP]);
        log::error!("RSP: {:#x}", state.gprs[regs::GPR_RSP]);
        log::error!("RAX: {:#x} RBX: {:#x} RCX: {:#x} RDX: {:#x}", state.gprs[regs::GPR_RAX], state.gprs[regs::GPR_RBX], state.gprs[regs::GPR_RCX], state.gprs[regs::GPR_RDX]);
        log::error!("RDI: {:#x} RSI: {:#x}", state.gprs[regs::GPR_RDI], state.gprs[regs::GPR_RSI]);
        log::error!("RFLAGS: {:#x}", state.gprs[regs::GPR_RFLAGS]);
        log::error!("CR0: {:#x} CR2: {:#x} CR3: {:#x} CR4: {:#x}", state.crs[regs::CR0], state.crs[regs::CR2], state.crs[regs::CR3], state.crs[regs::CR4]);
        log::error!("CS Attrib: {:#x}", state.segs[regs::SEG_CS].attrib);
        log::error!("EFER: {:#x}", state.msrs[regs::MSR_EFER]);
...
fn use_long_mode_pagetables(mem: &GuestMemoryMmap) -> io::Result<()> {
    ...
    mem.write_obj(pdpte_addr.raw_value() | 0x3, pml4_addr).unwrap();
    
    // Low Mapping (0-1GB) -> PDE (0xc000)
    // PDPTE[0] -> PDE | 0x3
    mem.write_obj(pde_addr.raw_value() | 0x3, pdpte_addr).unwrap();

    // High Memory / Kernel Space Mapping (-2GB)
    // Virtual 0xffffffff80000000 -> Physical 0
    // PML4[511] -> PDPTE | 0x3 (Reuse 0xb000)
    mem.write_obj(pdpte_addr.raw_value() | 0x3, pml4_addr.checked_add(511 * 8).unwrap()).unwrap();
    
    // PDPTE[510] -> PDE | 0x3 (Reuse 0xc000)
    mem.write_obj(pde_addr.raw_value() | 0x3, pdpte_addr.checked_add(510 * 8).unwrap()).unwrap();
    
    // PDE[0..512] -> 2MB Pages | 0x83 (P|RW|PS) -> 0b10000011 
    for i in 0..512 {
        let entry = i << 21 | 0x83;
        mem.write_obj(entry, pde_addr.checked_add(i * 8).unwrap()).unwrap();
    }
